...
<!-- Clickable Skip Text (visible only if no submissions exist) -->
<div v-if="noSubmissions" class="text-center mt-4">
  <span 
    @click="handleSkip" 
    class="text-blue-600 hover:underline hover:text-blue-800 cursor-pointer transition-colors duration-200 inline-flex items-center"
  >
...
// Add computed property to check if there are no submissions
const noSubmissions = computed(() => pensionStore.submissions.length === 0);
...
const handleSkip = () => {
  // Add any skip logic here, such as redirecting the user:
